...
export class BackupService {
    ...
    constructor(private backupRepo: BackupRepository, private s3: S3Service) { }

    async getAllBackups(payload: { offset: number, limit: number, date?: string }) {
        try {
            ...
            return await this.backupRepo.paginate({ filterQuery: filters, offset, limit, returnKey: "backups", pipelines: [] });
        } catch (err) {
            throw err;
        }
    }
    ...
    private backupOptions(path: string): Array<string> {
        return [`--uri=${this.mongoPath}/${this.mongoDb}`, '--forceTableScan', `--archive=${path}`, '--gzip', '--excludeCollection=auditlogs'];
    }

    private restoreOptions(path: string): Array<string> {
        return [`--uri=${this.mongoPath}/${this.mongoDb}`, `--archive=${path}`, '--gzip'];
    }
    ...
    async newBackup(): Promise<void> {
        ...
        try {
            ...
            try {
                let backupProcess = spawn('mongodump', options);
                backupProcess.on('exit', async (code, signal) => {
                    if (code || signal) {
                        await this.backupRepo.findOneAndUpdate({ _id: backup._id }, { status: 'Failed' });
                ...
                await this.backupRepo.findOneAndUpdate({ _id: backup._id }, { status: 'Failed', reason: err.message });
    ...
    async deleteBackup(backupIds: string[]): Promise<void> {
        try {
            await this.backupRepo.deleteMany({ _id: { $in: backupIds } });
        } catch (err) {
            throw err;
        }
    }

    private async backupToS3(path: string, backupId: string | Types.ObjectId): Promise<void> {
        const backupfile = readFileSync(path);
   
        const file = await this.s3.uploadFile(backupfile, `backups/${path}`, true);
        await this.backupRepo.findOneAndUpdate({ _id: backupId }, { status: 'Success', file: file.url });
        fs.unlinkSync(path);
    }
    ...
    async restoreFromS3(backupId: string): Promise<void> {
        ...
        stream.on('finish', () => {
            try {
                ...
                throw new Error('Restore process couldn\'t be completed due to, ' + err.message)
            }
        })
    }
}
